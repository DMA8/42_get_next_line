...
/*   Updated: 2021/11/17 19:46:54 by syolando         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > OPEN_MAX)
		return (NULL);
	next_line = gnl_remainder_and_update_it(remainder[fd]);
	...
	while (nbytes_readed)
	{
		...
		if (nl_index(buff) == -1 && nbytes_readed)
			...
			return (finish_line(next_line, buff, nl_index(buff), remainder[fd]));
	}
	return (NULL);
}
